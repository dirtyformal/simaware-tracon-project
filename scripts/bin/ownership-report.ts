...
// contributor (with contributor-spreadsheet metadata) so the project lead can
// triage who to ask to fix what.
//
// "Bulk-move" commits — commits that touched a very large number of files in
// one go (typically structural reorganisations rather than authorship work) —
// are filtered out so they don't claim attribution they shouldn't have. Bulk
// commits are detected by file count, not by author; pass --bulk-threshold to
// tune. Files with no remaining attributable author after filtering are
// flagged as **orphaned** for the maintainer to reclaim.
//
// Usage:
